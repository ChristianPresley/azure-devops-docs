...
The default work folder for Managed DevOps Pools agents is typically `D:\a\_work` (or `C:\a\_work` if the pool's VM size doesn't have a `D` drive) for Windows agents or `/mnt/vss/_work` for Linux agents, and your pipeline can reference the working folder by using the `Agent.WorkFolder` [predefined variable](/azure/devops/pipelines/variables/reference). Configure the **Work folder** setting to override the default work folder for every agent image in your pool.
